var Item = function(data) {
    ...
    app.filesystem.root.getDirectory("files", { create: true }, function(dirEntry){
      dirEntry.getFile(file.file_hash + ".pdf", { create: false },
        ...
        function getFileError(event) {
          ...
          var message = $("<div/>")
            .addClass("message")
            .html("<p>Fetching the PDF file&hellip;<p><p class='info'>Choose 'Sync Files' on the <a href='/settings'>settings page</a> to pre-fetch all files.</p>");
  ...
  this.renderFile = function(messageNode, readerNode, contentNode, fileURL){
    if (messageNode.hasClass("has-pdf-plugin")) {
      ...
      var link = $("<a/>", { href: fileURL, rel: "external" })
        .addClass("download-pdf")
        .text("Download the PDF file for viewing externally.");
